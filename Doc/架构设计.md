...
确保框架在 .NET Framework 4.0 到 .NET 10.0 的全版本兼容。

### 8.2 目标框架

| 框架 | 项目 | 条件编译符号 |
|------|------|-------------|
| net40 | NewLife.Agent | `NET40` |
| net45 | NewLife.Agent | `NET45_OR_GREATER` |
| net461 | NewLife.Agent | `NET461_OR_GREATER` |
| netstandard2.0 | NewLife.Agent | `NETSTANDARD2_0` |
| netstandard2.1 | NewLife.Agent | `NETSTANDARD2_1` |
| net6.0+ | 两个项目 | `NET6_0_OR_GREATER` |
| net10.0 | 两个项目 | `NET10_0` |

`NewLife.Extensions.Hosting.AgentService` 仅支持 `netcoreapp3.1`+ 框架，因为它依赖 `Microsoft.Extensions.Hosting`。

### 8.3 降级策略

- 低版本 API 替代：如 `GCCollectionMode.Aggressive` 仅 `NET8_0_OR_GREATER`
- P/Invoke 条件编译：Windows 特定 API 用 `NET5_0_OR_GREATER` + `[SupportedOSPlatform("windows")]`
- 部分功能（如 Web 面板）通过 `#if !NET40` 排除在 net40 之外
